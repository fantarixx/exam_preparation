...
- <u>Директивы **`#ifndef`** и **`#define`**</u>:
	...
	***<u>Особенности:</u>***
  	- Макрос называют именем данного заголовочного файла, но только в `high_snake_case`, например:
...
### Linkage
***<u>Связывание</u>*** <mark>linkage</mark> – это свойство **идентификатора**, позволяющее компилятору в некоторых случаях создавать для нескольких одинаковых имен, объявленных в разных *единицах трансляции*, одну **общую сущность**. Вместе с *областью видимости* связывание определяет, из каких *единиц трансляции* и их блоков можно обратиться к сущности.
- ***<u>No linkage</u>*** - сущность доступна только в **текущем скоупе** (сущности не связываются не с чем).
	<u>Например</u>:
...
- ***<u>Internal linkage</u>*** - доступность из **текущей** *единицы трансляции*. 
...
- ***<u>External linkage</u>*** - доступность из **всех** *единиц трансляции*.
...
- ***<u>Module linkage</u>*** - видимость в **единицах трансляции**, находящихся в том же ***модуле*** что и сущность, имеющая данный модификатор.
...
- ***<u>static</u>*** - **static duration** и **internal linkage**.
- ***<u>extern</u>*** - **static duration** и **external linkage**.
- ***<u>thread_local</u>*** - **thread storage duration**.
- ***<u>mutable</u>*** - используется для обозначения нестандартного (изменяемого) поведения переменной-члена класса. Он позволяет изменять значение этого члена даже внутри `const`-методов.

<u>Пример ***Storage duration and linkage***</u>:
```Cpp
// math.h
#pragma once
...
### Ошибки компиляции
- ***<u>Синтаксические ошибки</u>*** - нарушение правил синтаксиса языка.
```Cpp
int main() {
...
```
- ***<u>Семантические ошибки</u>*** - нарушение логики языка.
```Cpp
String text = 42; // Ошибка: несовместимость типов (int → String)
```
- ***<u>Ошибки линковки</u>*** - нарушение связей объекта.
...
```
- ***<u>Ошибки в директивах препроцессора</u>*** - Проблемы в **макросах** или **include-файлах**.
...
```
- ***<u>Ошибки версии языка/компилятора</u>*** - использование возможностей, неподдерживаемых *компилятором*.
...
```
- ***<u>Конфликты имен</u>*** - Повторное использование *идентификаторов*.
...
```
- ***<u>Ошибки в шаблонах</u>*** <mark>Generics</mark> - Некорректное использование **обобщенного программирования**.
